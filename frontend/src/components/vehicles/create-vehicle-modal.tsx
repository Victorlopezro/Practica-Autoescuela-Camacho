...
export function CreateVehicleModal({ open, onClose, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const payload: { plate: string; type: string; itvExpiry?: string } = {
        plate: form.plate.toUpperCase().trim(),
        type: form.type,
      };
      if (form.itvExpiry) payload.itvExpiry = `${form.itvExpiry}T00:00:00.000Z`;
      await services.vehicle.create(payload);
      onSuccess();
      handleClose();
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Añadir Vehículo">
