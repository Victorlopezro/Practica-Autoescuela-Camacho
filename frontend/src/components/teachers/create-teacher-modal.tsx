...
export function CreateTeacherModal({ open, onClose, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      await services.teacher.createTeacher({
        name: form.name.trim(),
  ...
  function handleClose() {
    setForm({ name: '', lastName: '', username: '', password: '', email: '', phone: '', vehicleIds: [] });
    setFieldErrors({});
    setError('');
    onClose();
  }
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Añadir Profesor">
