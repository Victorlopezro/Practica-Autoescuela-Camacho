...
import type { VehicleDto, VehicleIncidentDto } from '@/services/interfaces';
  ...
  return (
    <div className="space-y-6">
      <div className="bg-surface-container-low rounded-xl p-4 text-body-sm text-on-surface-variant border border-outline-variant/20">
        ...
        Estado de vehículos, ITV e incidencias. Pulsa en un vehículo para ver sus incidencias.
      </div>
  ...
  return (
    <Card accent>
      ...
      <div className="flex items-center gap-3">
        <div
          className="w-12 h-12 bg-surface-container-high rounded-xl flex items-center justify-center cursor-pointer"
          onClick={onToggle}
        >
        ...
        <span className={`text-label-caps text-sm ${v.available ? 'text-primary' : 'text-on-surface-variant'}`}>
      ...
      {/* Incidents section */}
      {isExpanded && <IncidentsSection vehicleId={v.id} />}
    </Card>
  );
}

function IncidentsSection({ vehicleId }: { vehicleId: string }) {
