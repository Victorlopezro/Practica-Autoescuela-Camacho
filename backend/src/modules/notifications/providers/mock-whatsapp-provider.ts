...
import { NotificationProvider, NotificationSendParams, NotificationSendResult } from '../../../common/interfaces';
...
export class MockWhatsAppProvider implements NotificationProvider {
  ...
  async send(params: NotificationSendParams): Promise<NotificationSendResult> {
    this.logger.log(`[MOCK] Sending ${params.template} to ${params.to}: ${JSON.stringify(params.data)}`);
